...
public class ParametrizedInvalidityTest {
    ...
    public static Collection<Object[]> data() throws URISyntaxException, IOException {
        Path filePath = Paths.get(ClassLoader.class.getResource("/fi/invalid_numbers.txt").toURI());
        return Files.lines(filePath).map(ssn -> new Object[]{ssn}).collect(toList());
    }

    @Test
    public void testInvalidity() {
        assertFalse("Expected " + ssnInput + " to be not valid ssn.", forId(ssnInput).isValid(FinnishIdNumber::valid));
    }
